...
beforeAll(async () => {
  ...
  const action = await prisma.action.create({
    data: { label: reason.label, defaultArticleId: articleId, orgId, callReasonId: reasonId },
  })
  actionId = action.id
  ...
  const [admin, manager, agent] = await Promise.all([
    prisma.agent.create({
      data: { name: 'Admin', username: 'admin', passwordHash: hash, role: 'ADMIN', orgId },
    }),
    prisma.agent.create({
      data: { name: 'Manager', username: 'manager', passwordHash: hash, role: 'MANAGER', orgId },
    }),
    prisma.agent.create({
      data: { name: 'Agent', username: 'agent', passwordHash: hash, role: 'AGENT', orgId },
    }),
  ])
